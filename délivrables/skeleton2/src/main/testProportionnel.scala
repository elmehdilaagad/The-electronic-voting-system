...
object testProportionnel {
  ...
  //println(system.getGagnants.length);
    //println("Liste des candidat")
    //for(candidat <- partiA.listCandidat)
      //println(candidat.prenom);
    for(gagnants <- system.getGagnants){
      for(candidat <- gagnants._4)
        println(candidat.nom +" du "+gagnants._1.nom+" a obtenu un siege BRAVO !!!; ")
      //println(gagnants._1.nom + " a obtenu "+gagnants._2.intValue);
      //println()
    }
